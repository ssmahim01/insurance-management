...
// /** Step 7: fetch booking + doctor details, needed to build the room ID. */
...
const extractPrescriptionUrl = (
  ...
  const entry = prescriptions[0];
  return (
    entry.prescriptionURL ??  // <-- actual field Zaynax returns, was missing
    entry.prescriptionUrl ??
    entry.url ??
    entry.pdfUrl ??
    entry.fileUrl ??
